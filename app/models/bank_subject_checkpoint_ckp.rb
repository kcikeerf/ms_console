class BankSubjectCheckpointCkp < ActiveRecord::Base
  ...
  class << self
    ...
    def get_web_ckps(params)
      checkpoint_system_id = params[:checkpoint_system_id] || 1
      if checkpoint_system_id == 1
        Common::CheckpointCkp.ckp_types_loop {|dimesion| get_ckps_by_dimesion(params, dimesion,checkpoint_system_id) }
      else
        get_ckps_by_dimesion(params, "other", checkpoint_system_id)
      end
    end

    def get_ckps_by_dimesion(params, dimesion, checkpoint_system_id)
      ...
      dim_all_nodes = self.where(:subject => target_subject, :category => target_category, :dimesion => dimesion,  :checkpoint_system_id => checkpoint_system_id)
    ...
    def get_all_ckps_plus(subject,xue_duan,system_id=1, str_pid='')
      if system_id == 1
    ...
    def get_all_ckps_by_dimesion_plus(subject, xue_duan, system_id, dimesion, str_pid='', options={})
      ...
      ckps = where(subject: subject,category: xue_duan, checkpoint_system_id: system_id, dimesion: dimesion).order({rid: :asc})
      unless str_pid.blank?
        ckp = ckps.find_by(uid: str_pid)
        ckps = ckp ? ckp.children : []
      end
    ...
    def save_ckp params
      ...
      target_objs = where(subject: subject, category: category, dimesion: dimesion, checkpoint_system_id: checkpoint_system_id)
      ...
      if new_rid.present?
        new_ckp = self.new(dimesion: params[:dimesion],
                           ...
                           checkpoint_system_id: checkpoint_system_id,
                           is_entity: true)
    ...
    # 循坏生成指标
    def generate_ckp(subject, dimesion, ckp_arr, system_id=1, parent_rid = nil)
      ckp_arr.each do |ckp|
        ckp.symbolize_keys!
        name = ckp[:text]
        new_ckp = save_ckp(subject: subject, checkpoint: name, dimesion: dimesion, checkpoint_system_id: system_id, str_pid: parent_rid)
    ...
    def common_influence_area(begin_node, not_include_node=nil)
      ...
      if begin_node.parent.present?
        ...
        nodes = where(subject: begin_node.subject, category: begin_node.category, dimesion: begin_node.dimesion, checkpoint_system_id: begin_node.checkpoint_system_id)
      end
  ...
  def children
    get_nodes(rid.size, rid, subject, dimesion, category, checkpoint_system_id).not_equal_rid(rid)
  end
  ...
  #
  def families
    ( subject.nil? || category.nil? || checkpoint_system_id.nil?)? [] : self.class.where(subject: subject, category: category, checkpoint_system_id: checkpoint_system_id)
  end

  private

  def get_nodes(length, rid, subject, dimesion, category, system_id=1)
    self.class.where('subject = ? and dimesion = ? and category = ? and checkpoint_system_id = ? and left(rid, ?) = ?', subject, dimesion, category, system_id, length, rid)
  end 
